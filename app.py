...
DRADIS_LOGO = ASSETS_DIR / "DRADIS_logo.png"
AUIX_LOGO = ASSETS_DIR / "AUiX_logo.png"
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .auix-app { width:100%; max-width:100%; min-width:0; overflow-x:hidden; font-family: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; color: var(--text); }
  ...
  .layout { display:grid; grid-template-columns:minmax(0, 2fr) minmax(330px, .88fr); gap:14px; align-items:start; width:100%; min-width:0; }
  .layout > * { min-width:0; }
  .network-wrap { position:relative; width:100%; min-width:0; border:1px solid var(--border); border-radius:18px; background:var(--bg); overflow:hidden; }
  #network { width:100%; max-width:100%; height:auto; display:block; aspect-ratio: 900 / 800; }
  ...
  @media (max-width: 960px) {
    ...
    .network-wrap { order:1; width:100%; max-width:100%; margin:0; }
    .side { order:2; width:100%; max-width:100%; min-height:0; margin-top:12px; }
    #network { width:100%; max-width:100%; aspect-ratio:900 / 800; }
...
(() => {
  ...
  function resizeFrame() {
    try {
      const app = document.querySelector('.auix-app');
      const h = Math.ceil(Math.max(
        app ? app.getBoundingClientRect().height : 0,
  ...
  if ('ResizeObserver' in window) {
    const frameObserver = new ResizeObserver(() => window.setTimeout(resizeFrame, 20));
    frameObserver.observe(document.querySelector('.auix-app'));
  }
...
st.markdown(
    ...
    <style>
      ...
      .auix-brand-row .dradis-logo {{ width: 132px; max-width: 32vw; }}
      .auix-brand-row .auix-logo {{ width: 145px; max-width: 35vw; }}
      .auix-device-note {{
        text-align: center;
        color: #667085;
        font-size: 0.86rem;
      ...
      @media (max-width: 520px) {{
        .auix-brand-row {{ gap: 20px; }}
        .auix-brand-row .dradis-logo {{ width: 104px; }}
        .auix-brand-row .auix-logo {{ width: 112px; }}
      }}
    </style>
    <div class="auix-brand-row">
      <img class="dradis-logo" src="{_image_data_uri(DRADIS_LOGO)}" alt="DRADIS logo">
      <img class="auix-logo" src="{_image_data_uri(AUIX_LOGO)}" alt="AUiX logo">
    </div>
    <div class="auix-device-note">Best viewed on desktop or tablet.</div>
    """,
...
components.html(build_component(network), height=1650, scrolling=False)
